<%@ Page Title="Ortak Liste Oluştur" Language="C#" MasterPageFile="~/Site.Master"
    AutoEventWireup="true" CodeBehind="OrtakListe.aspx.cs"
    Inherits="Cinescope.OrtakListe" %>
...
<div class="tab-wrapper">
    <a href="Kullanici.aspx?id=<%= Request.QueryString["uid"] %>" class="tab-item">Profil</a>
    <a href="#" class="tab-item active">Ortak Liste Oluştur</a>
</div>
...
<div class="content-container">
    ...
    <div class="toggle-row">
        <div id="typeFilm" class="toggle-card active" onclick="setType('film')">Film</div>
        <div id="typeDizi" class="toggle-card" onclick="setType('dizi')">Dizi</div>
    </div>

    <div style="position:relative;">
        <input id="searchInput" class="form-control" placeholder="Ara..." autocomplete="off"/>
    ...
    searchInput.oninput = e => {
        ...
        fetch(`https://api.themoviedb.org/3/search/${endpoint}?api_key=${API_KEY}&language=tr-TR&query=${encodeURIComponent(q)}`)
            ...
            .then(d => {
                ...
                d.results.filter(x => x.poster_path).slice(0, 6).forEach(x => {
                    const title = x.title || x.name;
                    const div = document.createElement("div");
                    div.className = "search-item";
                    div.innerHTML = `<img src="${IMG + x.poster_path}" width="40"><span>${title}</span>`;
                    div.onclick = () => {
                        if (!selected.some(s => s.id === x.id && s.type === type)) {
                            selected.push({ id: x.id, type: type, poster: x.poster_path });
                            sync();
                        }
                        searchInput.value = "";
                        searchResults.style.display = "none";
